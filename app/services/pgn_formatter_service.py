...
class PgnFormatterService:
    ...
    def _remove_non_standard_tags(pgn_text: str) -> str:
        ...
        while i < len(pgn_text):
            # Preserve metadata-only lines as-is (running line index; avoid O(n²) recount).
            ...
            if pgn_text[i] == '{':
                ...
                if comment_depth > 0:
                    ...
                    comment_content_parts.append(ch)
                else:
    ...
    def format_pgn_to_html(
        ...
        # PGN menu: "Text" = show descriptions; "Symbols" = glyphs for NAGs 1–6 only ($n has no glyph → still show text).
        if pgn_notation_settings is not None:
            show_nag_text = bool(pgn_notation_settings.get('show_nag_text', True))
            use_symbols_for_common_nags = bool(pgn_notation_settings.get('use_symbols_for_nags', True))
        else:
            show_nag_text = bool(nags_config.get('show_text', True))
            use_symbols_for_common_nags = bool(annotations_config.get('use_symbols', True))
        ...
        # NAGs without a standard glyph always become parenthetical text (never raw $n in the pane).
                ...
                if match and match.start() == 0:
                    ...
                    if nag_number in NAG_TO_SYMBOL:
                        if use_symbols_for_common_nags:
                            # Convert to symbol (e.g., "$1" → "!"); styled in the annotation pass
                            nag_display = NAG_TO_SYMBOL[nag_number]
                        else:
                            nag_display = f" ({get_nag_text(nag_number)})"
                    else:
                        nag_display = f" ({get_nag_text(nag_number)})"
        ...
        # Wrap known parenthetical NAG meanings (symbols mode still emits text for $7+).
        ...
        if all_patterns:
            ...
            # Anchored at match position: whitespace + (meaning). Use match(), never search()
            # on a suffix — searching every character through long comments is O(n²).
            nag_text_pattern = re.compile(r'\s+\((' + nag_meanings_pattern + r')\)')
        else:
            nag_text_pattern = re.compile(r'(?!)')

        comment_color_css_for_nag = (
            f"color: rgb({comment_color[0]}, {comment_color[1]}, {comment_color[2]})"
        )
        result_parts = []
        i = 0
        in_tag = False
        ...
        nag_span_stack: List[str] = []

        def _recompute_nag_span_flags() -> None:
            nonlocal in_header_span, in_comment_span
            in_header_span = any(span == 'header' for span in nag_span_stack)
            in_comment_span = any(span == 'comment' for span in nag_span_stack)
        ...
        while i < len(formatted):
            if formatted[i] == '<':
                ...
                if i + 5 <= len(formatted) and formatted[i : i + 5] == '<span':
                    ...
                    if tag_end != -1:
                        tag_content = formatted[i : tag_end + 1]
                        if header_color_css in tag_content:
                            nag_span_stack.append('header')
                        elif comment_color_css_for_nag in tag_content:
                            nag_span_stack.append('comment')
                        else:
                            nag_span_stack.append('other')
                        _recompute_nag_span_flags()
                        result_parts.append(formatted[i : tag_end + 1])
                        i = tag_end + 1
                        continue
                # Other HTML tag (e.g. <a ...>): copy whole tag
                tag_end = formatted.find('>', i)
                if tag_end != -1:
                    result_parts.append(formatted[i : tag_end + 1])
                    i = tag_end + 1
            ...
            if in_header_span or in_comment_span:
                result_parts.append(formatted[i])
                i += 1
                continue

            # Only attempt NAG-text match where the pattern can start (whitespace).
            if formatted[i].isspace():
                match = nag_text_pattern.match(formatted, i)
                if match:
                    ...
                    if (
                        ...
                        and '(' in full_match
                        and ')' in full_match
                        and full_match.count('(') == 1
                        and full_match.count(')') == 1
                    ):
                        result_parts.append(
                            span(full_match, nag_color, nag_bold, nag_italic)
                        )
                        i = match.end()
                        continue
        ...
        while i < len(formatted):
            if formatted[i] == '<':
                ...
                # Look for move SAN match starting at position i (match, not search —
                # search would scan through following comment HTML on every character).
                match = PGN_MAINLINE_SAN_PATTERN.match(formatted, i)
                if match:
        ...
        # Each pattern maps to the NAG id used for "Text" mode (PGN menu) descriptions.
        annotation_patterns = [
        ...
        annotate_move_assessments_as_text = show_nag_text and not use_symbols_for_common_nags
        ...
        compiled_patterns = [
            (re.compile(pattern), color, is_bold, nag_id)
            for pattern, color, is_bold, nag_id in annotation_patterns
        ]
        ...
        while i < len(formatted):
            if formatted[i] == '<':
                ...
                matched = False
                for compiled_pattern, color, is_bold, nag_glyph_id in compiled_patterns:
                    match = compiled_pattern.match(formatted, i)
                    if match:
                        if annotate_move_assessments_as_text:
                            nag_txt = get_nag_text(nag_glyph_id)
                            text_out = f" ({nag_txt})"
                            annotation_formatted = span(text_out, nag_color, nag_bold, nag_italic)
                        else:
